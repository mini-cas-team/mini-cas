...
export default function TranscriptsTab() {
    ...
    const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files && e.target.files.length > 0) {
            ...
            // Assumes a 'transcripts' bucket exists in Supabase
            ...
            if (error) {
                ...
                alert('Failed to upload file. Does the "transcripts" storage bucket exist?');
            } else {
                const newTranscript = {
                    name: file.name,
                    path: data.path,
                    ...
                    transcripts.map((transcript, index) => (
                        <div key={index} className="flex items-center justify-between p-4 bg-gray-50 rounded-xl border border-gray-100 transition-all hover:border-gray-300">
                            <div className="flex items-center gap-4">
                                <a
                                    href={supabase.storage.from('transcripts').getPublicUrl(transcript.path).data.publicUrl}
                                    target="_blank"
